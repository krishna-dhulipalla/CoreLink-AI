...
from engine.agent.context.evidence import (
...
from engine.agent.context.extraction import extract_entities, extract_formulas, extract_inline_facts, extract_urls, parse_markdown_tables
from engine.agent.context.profiling import _extract_labeled_json_block
from engine.agent.benchmarks.officeqa import officeqa_analysis_modes
...
from engine.agent.contracts import CuratedContext, EvidenceSufficiency, RetrievalIntent, ReviewPacket, SourceBundle, TaskIntent
from engine.agent.retrieval_reasoning import assess_evidence_sufficiency, build_retrieval_intent
...
def build_curated_context(
    ...
    curated = CuratedContext(
        ...
        provenance_summary={
            "benchmark_adapter": str(overrides.get("benchmark_adapter", "") or ""),
            "source_bundle": {
            ...
            "retrieval_plan": {
                ...
                "granularity_requirement": retrieval_intent_obj.granularity_requirement if retrieval_intent_obj else "",
                ...
                "decomposition_confidence": retrieval_intent_obj.decomposition_confidence if retrieval_intent_obj else 0.0,
                "decomposition_used_llm_fallback": retrieval_intent_obj.decomposition_used_llm_fallback if retrieval_intent_obj else False,
...
def attach_structured_evidence(
    ...
    overrides = dict(benchmark_overrides or {})
    structured = benchmark_build_structured_evidence(tool_results, benchmark_overrides)
    ...
    if structured.get("values"):
        compact_structured = benchmark_compact_structured_evidence(structured, benchmark_overrides)
        facts.append(
            {
                "type": "structured_sample_values",
                "value": compact_structured.get("values", [])[:6],
            }
        )

    provenance_summary = dict(curated.provenance_summary or {})
    if overrides.get("benchmark_adapter"):
        provenance_summary["benchmark_adapter"] = str(overrides.get("benchmark_adapter", "") or "")
...
def attach_compute_result(
    ...
    curated = curated_context if isinstance(curated_context, CuratedContext) else CuratedContext.model_validate(curated_context)
    compact = benchmark_compact_compute_result(compute_result, benchmark_overrides)
    if not compact:
        return curated
...
def build_review_packet(
    *,
    ...
    benchmark_overrides = {"benchmark_adapter": str(provenance.get("benchmark_adapter", "") or "")}
    ...
    return ReviewPacket(
        ...
        structured_evidence=benchmark_compact_structured_evidence(curated_context.get("structured_evidence", {}), benchmark_overrides),
        compute_result=benchmark_compact_compute_result(curated_context.get("compute_result", {}), benchmark_overrides),
