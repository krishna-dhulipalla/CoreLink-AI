...
from engine.agent.langsmith_env import normalize_langsmith_env
from engine.agent.cost import get_active_cost_tracker
...
def get_client_kwargs(role: str) -> dict[str, Any]:
    ...
    if not api_key:
        if is_nebius_profile:
            ...
            # Fall back to NEBIUS_API_KEY if OPENAI_API_KEY is not explicitly provided.
            api_key = os.getenv("OPENAI_API_KEY") or os.getenv("NEBIUS_API_KEY")

    if not base_url:
        if is_nebius_profile:
            base_url = os.getenv("NEBIUS_BASE_URL") or "https://api.studio.nebius.ai/v1/"
        else:
            base_url_override = os.getenv("OPENAI_BASE_URL") or os.getenv("NEBIUS_BASE_URL")
            if base_url_override:
                base_url = base_url_override
            elif not os.getenv("OPENAI_API_KEY") and os.getenv("NEBIUS_API_KEY"):
                # If we fell back to the Nebius key because OpenAI was missing, 
                # we must also fall back to the Nebius API host.
